...
describe('test ido', async () => {
  ...
  before(async () => {
    ...
    expect((launchpadData.result[2] as Slice).loadAddress().toString()).to.be.eq(sourceJetton.address.toString());
  ...
  it('participate', async () => {
    ...
    const {balance, owner: _launchpad} = parseJettonWalletDetails(
