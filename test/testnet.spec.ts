...
describe("test contract at test net", async function () {
  it("init", async () => {
    ...
    const res = await timeLock.contract.sendInternalMessage(
      internalMessage({
        from: owner,
        value: toNano("0.1"),
        body: beginCell()
          .storeAddress(timeLockJWallet.address) // let time lock to invoke account time lock sold jetton wallet to release jetton
          .storeRef(JettonWallet.transferBody(owner, toNano("2")))
          .endCell(),
      })
    );
